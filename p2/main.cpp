  ...
  for(int last = number_of_videos -1; last > 0; last--)
    for (int cur = 0; cur < last; cur++)
      if (video_info[cur]->lower(video_info[cur+1]))
        swap(video_info[cur], video_info[cur+1]);  // since videos is an array of pointers you can simply swap the addresses at the cur and cur+1 locations.if (sort==rating) 
  ...
  for(int last = number_of_videos -1; last > 0; last--)
    for (int cur = 0; cur < last; cur++)
      if (video_info[cur]->alph(video_info[cur+1]))
        swap(video_info[cur], video_info[cur+1]);  // since videos is an array of pointers you can simply swap the addresses at the cur and cur+1 locations.if (sort==title) 
